...
public class Knife extends Weapon implements UpgradeableItem {
    ...
    public Knife(Planet planet) throws IOException {
        ...
        if (count > 0) temp = random.nextInt(26);
        damage = temp;
        if (count - temp > 0) count -= temp;
        else {
            count = 0;
        ...
        if (count > 0) temp = random.nextInt(51);
        speed = temp;
        if (count - temp > 0) count -= temp;
        else {
            count = 0;
    ...
    public String toString() {
        String string = String.format("%nnome: %s %nvalori del coltello:%nresistenza: %d%nprotezione: %d%nforza: %d%nvelocit�: %d"
