...
public class Sword extends Weapon implements UpgradeableItem {
    ...
    public Sword(Planet planet) throws IOException {
        ...
        startResistance();
        resistance += 1200;
        if (count > 0) temp = random.nextInt(21);
        defence += temp;
        ...
        if (count > 0) temp = random.nextInt(31);
        damage = temp;
        if (count - temp > 0) count -= temp;
        else {
            count = 0;
        ...
        if (count > 0) temp = random.nextInt(11);
        speed = temp;
        if (count - temp > 0) count -= temp;
        else {
            count = 0;
        ...
        count = 20;
        if (count > 0) temp = random.nextInt(10);
        blade[0] = temp;
        if (gold > 10) blade[0] += (gold - temp);
        if (count - temp > 0) count -= temp;
        else {
            count = 0;
            temp = 0;
    ...
    public String toString() {
        String string = String.format("%nnome: %s %nvalori della spada:%nresistenza: %d%nprotezione: %d%nforza: %d%nvelocit�: %d"
    ...
    private void drawImage() throws IOException {
        ...
        try {
            ImageIO.write((RenderedImage) weapon, "png", bos);
        } catch (IOException e) {
            e.printStackTrace();
        }
