...
public class Sword extends Weapon implements UpgradeableItem {
	/**
	 * 
	 */
	private static final long serialVersionUID = 8076462405621935934L;
	private String type = "";
	...
	public Sword(Planet planet) throws IOException {
		...
		startResistance();
		resistance+=1200;
		if(count>0)temp = random.nextInt(21);defence+=temp; if(count-temp>0)count-=temp; else {count=0;temp=0;}
		if(count>0)temp = random.nextInt(31);damage=temp;if(count-temp>0)count-=temp; else {count=0;temp=0;}
		if(count>0)temp = random.nextInt(11);speed=temp;if(count-temp>0)count-=temp; else {count=0;temp=0;}
		...
		if(bladeP[2]>40f) {resistance+=400;defence+=10;speed-=20;type+="_strong_";}
	...
	public String toString() {
		String string = String.format("%nnome: %s %nvalori della spada:%nresistenza: %d%nprotezione: %d%nforza: %d%nvelocità: %d"
	...
	private void drawImage() throws IOException {
		weapon = ImageIO.read(new File("models/sword.png"));
