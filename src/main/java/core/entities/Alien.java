...
public class Alien extends Monster {
	...
	public Alien(Planet planet) {
		...
		if(count>0)temp = random.nextInt(15);this.strength=temp; if(count-temp>0)count-=temp; else {count=0;temp=0;}
		if(count>0)temp = random.nextInt(15);this.protection=temp;if(count-temp>0)count-=temp; else {count=0;temp=0;}
		if(count>0)temp = random.nextInt(15);this.agility=temp;if(count-temp>0)count-=temp; else {count=0;temp=0;}
		if(count>0)temp = random.nextInt(15);this.fear=temp;if(count-temp>0)count-=temp; else {count=0;temp=0;}
		if(count>0)temp = random.nextInt(15);this.anger=temp;if(count-temp>0)count-=temp; else {count=0;temp=0;}
		
		if(planet.getValue(Valore.FERRO)>40) {this.protection+=5;this.type+="_iron_";}
		if(planet.getValue(Valore.ORO)>30) {this.gold+=200;this.type+="_gold_";}
		if((planet.getValue(Valore.ACQUA)<20) && (planet.getValue(Valore.TERRA)>40)) {this.agility+=6;this.type+="_fast_";}
		name="Alien"+type;
